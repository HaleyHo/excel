...
import tkinter as tk #創建一個控件
import pandas as pd
from tkinter import filedialog #打開文件框
from tkinter import ttk#下拉列表
from tkinter import messagebox#打開對話框
from tkinter import *#導入方式
import time
import os
import threading
from single_material import component,write_sheet

class UI_dataanalysis_calss:
    def __init__(self, window):
        self.window = window
		#界面的标题及尺寸不可变
        self.window.title('数据处理软件')#設置窗口標題
        self.window.geometry('+700+500')#設置大小和位置
        self.window.resizable(False, False)#是否可以重新設置大小
    ...
    def Create_window(self):
		...
		#创建一个一级窗口
        self.labelframe_1 = tk.LabelFrame(self.window,  text='文件处理：',padx=20,pady=10,relief=FLAT)
        self.labelframe_1.pack()#pady=10
        self.labelframe_2 = tk.LabelFrame(self.window,  text='选择的选项：',padx=10,pady=10,relief=FLAT)
        self.labelframe_2.pack()
        self.labelframe_3 = tk.LabelFrame(self.window,  text='请选择需要生成的表格：',padx=10,pady=10,relief=FLAT)
        ...
        self.open_file_path_button = tk.Button(self.labelframe_1,  text='路径选择',relief=RAISED,  command=self.election_file_directory_1)
        ...
        self.open_file_path_button = tk.Button(self.labelframe_1,  text='路径选择',relief=RAISED,  command=self.election_file_directory_2)
        ...
        self.save_file_path_button = tk.Button(self.labelframe_1,  text='文件保存', relief=RAISED, command=self.save_file_directory)
        ...
        self.search_button = tk.Button(self.labelframe_1,  text='搜索', relief=RAISED, state='disable',command=self.search_action)
        self.search_button.grid(row = 3,  column=2)

		#点击分析处理文件
        self.bit_analysis = tk.Button(self.labelframe_1,  text='点击进行分析',relief=RAISED, state='disable', command=self.progressbar_1)
        self.bit_analysis.grid(row = 4, column = 2)
        #点击生成表格
        self.button_2 = tk.Button(self.labelframe_1,  text='生成表格',relief=RAISED,state='disable', command=self.sheet)
    ...
    def search_action(self):
        ...
        # self.button_1_label = tk.Label(self.labelframe_1, text='数据分析中,请选择需要的选项', background='wheat')
        # self.button_1_label.grid(row=4, column=0)
        values = self.search.get()
        if values == '':
            # self.button_1_label = tk.Label(self.labelframe_1, text='      请输入需要搜索的内容      ', background='red',fg='White')
            # self.button_1_label.grid(row=4, column=0)
            tk.messagebox.showerror(title='error', message='请输入需要搜索的内容')
            return
        else:
            list_component_search = []
            data_analysis_judge_1,self.sheet_data = component(self.election_file_path_1.get())
            for i in data_analysis_judge_1:
                if str.lower(values) in str.lower(i):
                    list_component_search.append(i)
            if list_component_search == []:
                # self.button_1_label = tk.Label(self.labelframe_1, text='  没有搜索到该值,请重新输入  ', background='red',fg='White')
                # self.button_1_label.grid(row=4, column=0)
                ...
                for i in range(len(list_component_search)):
                    ...
                    elect_1.grid(row=row_1, column=column_1, sticky=W)
                    elect_1.deselect()
                #创建条件全选快捷按键及全部不选的变更同时生成下一步数据可视化化的按键,同时激活生成表格按键
                self.button_1 = tk.Button(self.labelframe_3,  text='条件全选',relief=RAISED, command = self.selectall)
                self.button_1.grid(row=row_1+1, column=0)
                self.button_confirm = tk.Button(self.labelframe_3,  text='确定',relief=RAISED, command=self.confirm)
    ...
    def progressbar_1(self):
        # self.button_1_label = tk.Label(self.labelframe_1, text='数据分析中,请选择需要的选项', background='wheat')
        # self.button_1_label.grid(row=4, column=0)
        # self.bit_analysis.config(state='disable')
    ...
    def confirm(self):
        ...
        for i in range(len(self.confirm_list)):
            if i == 0:
                ...
                row_1 = int(i/5)+3
                column_1 = i%5
            tk.Label(self.labelframe_2,  text='['+self.confirm_list[i]+']').grid(row=row_1, column=column_1, sticky=W)
    ...
    def sheet(self):
        ...
        if len(self.confirm_list) == 0:
            ...
            write_sheet(self.election_file_path_2.get(),self.sheet_data,self.confirm_list,self.save_file_path.get())
            tk.messagebox.showinfo(title='successful', message='数据分析完成，表格已生成')
            #生成选择再次运行和退出程序选项
            self.labelframe_4 = tk.LabelFrame(self.window,  text='表格已生成，选择再次运行程序或是退出程序：',padx=0,pady=10,relief=FLAT)
            self.labelframe_4.pack()#pady=10
            self.button_3 = tk.Button(self.labelframe_4, text='再次运行', relief=RAISED, state='disable', command=self.again_run)
            self.button_3.grid(row=0, column=0, padx=50)
            self.button_4 = tk.Button(self.labelframe_4, text='退出程序', relief=RAISED, state='disable', command=lambda:exit())
